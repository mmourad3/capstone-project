...
export function DormSeekerDetailModal({
  ...
  const [requestedToThisUser, setRequestedToThisUser] = useState(false);
...
useEffect(() => {
  ...
  const loadStatus = async () => {
    try {
      ...
      setRequestedToThisUser(
        status.pendingRequest &&
          status.pendingRequest.recipientId === selectedListing.posterId &&
          status.pendingRequest.dormId === selectedListing.id,
      );
    } catch (error) {
  ...
  window.addEventListener("roommateDataChanged", loadStatus);

  return () => {
    window.removeEventListener("roommateDataChanged", loadStatus);
  };
}, [selectedListing]);
      ...
      >
        ...
        <div className="p-4 sm:p-6">
          ...
          <div className="mb-6">
            ...
            <div className="grid sm:grid-cols-2 gap-4 mb-4">
              <div className="text-gray-700 dark:text-gray-300">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400 ml-6">
                  {formatDistance(selectedListing.distanceKm)} from{" "}
                  {userUniversity}
                </p>
              </div>
              <div className="text-gray-700 dark:text-gray-300">
            ...
            <div className="mb-4">
              ...
              <div className="flex flex-wrap gap-2">
                {selectedListing.amenities.map((amenityId, idx) => {
                  const amenityInfo = availableAmenities.find(
                    (a) => a.id === amenityId,
                  );
                  ...
                  return (
                    <span
                      key={idx}
                      className="bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 px-3 py-1 rounded-full text-sm flex items-center gap-1.5"
                    >
            ...
            {/* Lifestyle Tags */}
            {selectedListing.lifestyleTags &&
              selectedListing.lifestyleTags.length > 0 && (
                <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4">
