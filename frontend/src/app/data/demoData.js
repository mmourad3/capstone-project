...
const getUniversityEmailDomain = (universityName) => {
  ...
  for (const [domain, expectedUniversity] of Object.entries(EMAIL_UNIVERSITY_MAP)) {
    ...
    if (universityName === expectedUniversity || universityName.startsWith(expectedUniversity)) {
      return domain;
    }
  }
...
export const DEMO_USERS = [
  ...
  },
  {
    ...
    phone: '+9611234577',
    gender: 'male',
    profilePicture: 'https://images.unsplash.com/photo-1531427186611-ecfd6d936c79?w=400',
    country: 'Lebanon',
    countryCode: 'LB',
    countryISO: 'LB',
    university: 'American University of Beirut',
  ...
  },
  {
    ...
    phone: '+9611234579',
    gender: 'male',
    profilePicture: 'https://images.unsplash.com/photo-1506277886164-e25aa3f4ef7f?w=400',
    country: 'France',
    countryCode: 'FR',
    countryISO: 'FR',
    university: 'Université Saint-Joseph',
...
export const DEMO_QUESTIONNAIRES = {
  ...
  'user-rami-006': {
    ...
    importantQualities: ['Communicative', 'Respectful', 'Friendly', 'Adaptable'],
    dealBreakers: ['Smoking', 'Messiness']
  },
  'user-alex-009': {
    sleepSchedule: 'Balanced',
...
export const DEMO_DORM_LISTINGS = [
  ...
  },
  {
    ...
    roomType: "Single Room",
    genderPreference: "same", // Provider (Male) accepts same gender only
    description: "Comfortable single room near AUB campus. Perfect for active students who enjoy sports and outdoor activities. Includes gym access in building. Quiet neighborhood ideal for studying with easy access to campus.",
    amenities: ['wifi', 'ac', 'furnished', 'desk', 'utilities', 'gym', 'parking'],
    images: [
    ...
    posterGender: "Male",
    posterProfilePic: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=400",
    country: "Lebanon",
    countryCode: "LB",
...
export const DEMO_CARPOOL_LISTINGS = [
  {
    ...
    driverGender: 'Male',
    driverProfilePicture: 'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?w=400',
    university: 'American University of Beirut',
    region: 'Jounieh',
  ...
  },
  {
    ...
    driverCountryCode: '+961',
    driverGender: 'Female',
    driverProfilePicture: 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=400',
    university: 'Lebanese American University',
    region: 'Byblos',
  ...
  },
  {
    ...
    driverCountryCode: '+961',
    driverGender: 'Male',
    driverProfilePicture: 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=400',
    university: 'Université Saint-Joseph',
    region: 'Tripoli',
...
export function generateDemoCarpools(userRegion, userUniversity) {
  ...
  const userClassSchedule = JSON.parse(localStorage.getItem('classSchedule') || '[]');
  
  console.log('🚗 Generating SMART carpools for user schedule:', userClassSchedule);
  ...
  const carModels = [
    'Honda Civic 2020', 'Toyota Corolla 2019', 'Hyundai Elantra 2021', 'Kia Cerato 2020',
    'Nissan Sunny 2018', 'BMW 320i 2019', 'Mercedes C-Class 2020', 'Chevrolet Cruze 2019',
    'Ford Focus 2018', 'Mazda 3 2021'
  ];
  
  const pickupSpots = [
    'Main Square', 'Bus Station', 'City Center', 'Near the Mosque', 'Shopping Mall Entrance',
    'Gas Station', 'Public Library', 'Town Hall', 'Central Park', 'Hospital Parking'
  ];
  ...
  const generateDepartTimes = (startTime) => {
    ...
    for (let offset of [90, 60, 30, 0]) {
      ...
      let m = min - offset;
      while (m < 0) { m += 60; h -= 1; }
      if (h >= 5) times.push(`${String(h).padStart(2, '0')}:${String(m).padStart(2, '0')}`);
    }
  ...
  const generateReturnTimes = (endTime) => {
    ...
    for (let offset of [0, 30, 60, 90]) {
      ...
      let m = min + offset;
      while (m >= 60) { m -= 60; h += 1; }
      if (h < 22) times.push(`${String(h).padStart(2, '0')}:${String(m).padStart(2, '0')}`);
    }
  ...
  const getAllSubsets = (arr) => {
    ...
    for (let i = 1; i < total; i++) { // Start from 1 to exclude empty set
      const subset = [];
  ...
  userClassSchedule.forEach((block, blockIdx) => {
    ...
    dayCombinations.forEach((dayCombo, comboIdx) => {
      ...
      const carpoolsPerCombo = Math.min(4, Math.ceil(TARGET_CARPOOLS / (userClassSchedule.length * dayCombinations.length)));
      ...
      for (let i = 0; i < carpoolsPerCombo; i++) {
        ...
        for (let j = 0; j < numPassengers; j++) {
          const randomPassenger = passengersPool[(globalIdx + j) % passengersPool.length];
          passengers.push({
            ...
            joinedAt: new Date(today.getTime() - (globalIdx + j) * 3600000).toISOString()
          });
        }
        ...
        demoCarpools.push({
          ...
          price: parseFloat((5.00 + (globalIdx % 10) * 2.00).toFixed(2)), // DECIMAL: 5.00, 7.00, 9.00, 11.00, etc
          carModel: carModel,
          genderPreference: genderPreference,
          passengers: passengers,
          status: carpoolStatus, // 'Full' if no seats left, 'Active' otherwise
          createdAt: new Date(today.getTime() - globalIdx * 3600000).toISOString()
        });
  ...
  console.log(`✅ Generated ${demoCarpools.length} SMART carpools with NO overlapping driver days!`);
  return demoCarpools;
}
...
export const getCarpoolListingsByDriverId = (driverId) => {
  return DEMO_CARPOOL_LISTINGS.filter(carpool => carpool.driverId === driverId);
};
...
export const loadDemoUser = (emailOrId) => {
  ...
  try {
    ...
    if (user.questionnaire) {
      ...
      localStorage.setItem(questionnaireKey, JSON.stringify(user.questionnaire));
...
export const loadDemoCarpoolListings = () => {
  try {
    ...
    console.log(`✅ Loaded ${DEMO_CARPOOL_LISTINGS.length} demo carpool listings`);
    return true;
  } catch (error) {
